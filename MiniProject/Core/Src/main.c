...
int main(void)
{
  ...
  while (1)
  {
	val = HAL_GPIO_ReadPin(Sensor_Input_GPIO_Port, Sensor_Input_Pin);
	if(HAL_GPIO_ReadPin(Sensor_Input_GPIO_Port, Sensor_Input_Pin)==1)
	{
	  HAL_GPIO_TogglePin(Green_Led_GPIO_Port, Green_Led_Pin);
	  HAL_Delay(50);
	  while(HAL_GPIO_ReadPin(Sensor_Input_GPIO_Port, Sensor_Input_Pin)==0)
	  {
	    HAL_GPIO_TogglePin(Blue_Led_GPIO_Port, Blue_Led_Pin);
	    HAL_Delay(50);
	  }
	}
	HAL_SPI_Transmit(&hspi2, &val, 10, 100);
	//HAL_Delay(1000);
  }
  /* USER CODE END 3 */
}
